...
| refactor | 行为不变的重构 |
| perf | 性能 |
| style | 仅代码格式、空白、排版等不影响运行结果的改动 |
| docs | 文档 |
| chore / ci / build | 工具链、依赖、构建 |

**UI 改动的 type 判定**：

- `style` 不表示界面样式。CSS、颜色、间距、布局、按钮顺序、图标和动效等用户可见变化，不要仅因“样式调整”使用 `style`
- 修正与设计稿不一致、布局错位、视觉缺陷、交互顺序或可用性问题，使用 `fix`
- 新增用户可见的界面能力、交互方式或组件功能，使用 `feat`
- 仅整理 UI 实现且渲染结果、交互行为均不变，使用 `refactor`
- 意图不明确时，结合 diff、相关上下文和近期提交判断；无法确认是新增能力时优先使用 `fix`，并在正文说明修正内容
